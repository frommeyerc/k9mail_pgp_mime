...
class StoreSchemaDefinition implements LockableDatabase.SchemaDefinition {
	...
	StoreSchemaDefinition(LocalStore localStore, Account account) {
    ...
    private void upgradeDatabase(final SQLiteDatabase db) {
        ...
        AttachmentProvider.clear(localStore.mApplication);
            ...
            if (db.getVersion() < 29) {
                ...
                if (db.getVersion() < 43) {
                    try {
                        ...
                        LocalFolder oldOutbox = new LocalFolder(localStore, account, "OUTBOX");
                        if (oldOutbox.exists()) {
                        ...
                        LocalFolder obsoleteOutbox = new LocalFolder(localStore, account, localizedOutbox);
                        if (obsoleteOutbox.exists()) {
                            ...
                            if (messages.length > 0) {
                                ...
                                LocalFolder drafts = new LocalFolder(localStore, this.account, this.account.getDraftsFolderName());
    ...
    private void update41Metadata(final SQLiteDatabase  db, SharedPreferences prefs, int id, String name) {
        ...
        if (this.account.getInboxFolderName().equals(name)) {
