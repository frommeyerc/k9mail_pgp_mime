...
public class LocalFolder extends Folder implements Serializable {
    ...
    private <T> T dbExecute(DbCallback<T> callback) throws MessagingException {
        try {
            return this.localStore.database.execute(false, callback);
        } catch (WrappedException e) {
            throw (MessagingException) e.getCause();
        }
    }

    private <T> T dbExecuteTransactional(DbCallback<T> callback) throws MessagingException {
        try {
            return this.localStore.database.execute(true, callback);
        } catch (WrappedException e) {
            throw (MessagingException) e.getCause();
        }
    }
    ...
    public boolean exists() throws MessagingException {
        return dbExecute(new DbCallback<Boolean>() {
            @Override
    ...
    public int getMessageCount() throws MessagingException {
        return dbExecute(new DbCallback<Integer>() {
            @Override
    ...
    public String getMessageUidById(final long id) throws MessagingException {
        return dbExecute(new DbCallback<String>() {
            @Override
    ...
    public LocalMessage getMessage(final String uid) throws MessagingException {
        return dbExecute(new DbCallback<LocalMessage>() {
            @Override
    ...
    public Message[] getMessages(final MessageRetrievalListener listener, final boolean includeDeleted) throws MessagingException {
        return dbExecute(new DbCallback<Message[]>() {
            @Override
    ...
    public Message storeSmallMessage(final Message message, final Runnable runnable) throws MessagingException {
        return dbExecuteTransactional(new DbCallback<Message>() {
            @Override
            public Message doDbWork(final SQLiteDatabase db) throws WrappedException, UnavailableStorageException {
                try {
    ...
    public void destroyMessages(final Message[] messages) throws MessagingException {
        dbExecuteTransactional(new DbCallback<Void>() {
            @Override
            public Void doDbWork(final SQLiteDatabase db) throws WrappedException, UnavailableStorageException {
                for (Message message : messages) {
                    try {
    ...
    public List<Message> extractNewMessages(final List<Message> messages)
            throws MessagingException {
        return dbExecute(new DbCallback<List<Message>>() {
            @Override
